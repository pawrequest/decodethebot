...
from episode_ext import EpisodeExt
...
async def episode_bot(session):
    ep_bot: EpisodeBot = await EpisodeBot.from_url(MAIN_URL, session, episode_db_type=EpisodeExt)
    return ep_bot
...
async def test_gets_episodes_and_skip_existing(episode_bot, session):
    ...
    eps = session.exec(select(EpisodeExt)).all()
    assert e1 in eps
