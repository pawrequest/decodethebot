...
from pawsupport.misc import snake_name
...
def filter_factory(model, url, placeholder):
...
def make_route(clazz, tag_model:RoutableModel):
    @router.get(f"/{tag_model.rout_prefix()}", response_model=FastUI, response_model_exclude_none=True)
    def smth_list_view(
